...
find_world_writable() {
    ...
    # Instructions:
    # 1. Use 'find' to search $TEST_DIR for items with world-write permission (-perm -002)
    # 2. For each item, check if it's a file (-f) or directory (-d), get permissions (%a), and print formatted output.
    # 3. Use process substitution: done < <(find ...) to preserve the count.
...
find_executable_non_scripts() {
    ...
    #    Use -type f, \( -name ... -o ... \), and -perm /111.
    # 2. For each file, get permissions (%a), print formatted output, and increment count.
    # 3. Use process substitution: done < <(find ...) to preserve the count.
